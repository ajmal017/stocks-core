...
public class IndicatorChartTest {
    ...
    public void cloneTest() {
        ...
        try {
            ...
            assertArrayEquals("parameters", i1.getParams().toArray(), i2.getParams().toArray());

            // Change original and verify cloned copy does not change
            c1.getIndicator().setParams(99,2,3);
            c1.getOverlay(0).setParams(39);
            c1.addOverlay(new ExpMovingAverage(55));

            // Verify current
            assertEquals("original parameters", 99, i1.getParams().get(0));
            assertEquals("original overlay parameter", 39, c1.getOverlay(0).getParams().get(0));
            assertEquals("original 2nd overlay", 2, c1.getOverlayCount());

            // Verify clone
            assertEquals("cloned parameters", 1, i2.getParams().get(0));
            assertEquals("cloned overlay parameter", 45, c2.getOverlay(0).getParams().get(0));
